// ============================================================================
// PULSE ECP — Такском Provider (Stub)
// ============================================================================

import type { ECPProvider, ECPProviderConfig, Certificate, SignResult, VerifyResult, ProviderInfo } from '../types'

export class TaxcomProvider implements ECPProvider {
  readonly name = 'taxcom'
  readonly displayName = 'Такском'
  ...
  async signDocument(documentId: string, certId: string): Promise<SignResult> {
    return {
      ...
      signatureHex: `tc_${Date.now()}_${documentId}_signed`,
      signedAt: new Date(),
      status: 'valid',
  ...
  async verifySignature(documentId: string): Promise<VerifyResult> {
    return {
      ...
      signerCertificate: {
        id: 'cert-stub',
        serialNumber: 'TC0123456789',
        subjectName: 'CN=Тестовый пользователь Такском',
        issuerName: 'CN=УЦ Такском',
  ...
  getProviderInfo(): ProviderInfo {
    return {
      ...
      isQualified: true,
      features: ['Подписание документов', 'ЭДО', 'Отправка отчётности'],
    }
  }
}
